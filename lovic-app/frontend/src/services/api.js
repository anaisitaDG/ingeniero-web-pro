...
export const api = {
  ...
  food: {
    ...
    insights:  ()        => request('/food/insights'),
  ...
  progressPhotos: {
    ...
    compare:        (register_a, register_b, user_id, refresh) => request('/progress-photos/compare', { method: 'POST', body: JSON.stringify({ register_a, register_b, user_id, refresh }) }),
  ...
  workout: {
    ...
    deleteActivity: (day_id, type) => request('/workout/activity', { method: 'DELETE', body: JSON.stringify({ day_id, type, date: new Date().toLocaleDateString('en-CA') }) }),
  ...
  trainer: {
    ...
    saveWorkout:     (id, days, duration_days, start_date, plan_id, name)  => request(`/trainer/clients/${id}/workout`, { method: 'PUT', body: JSON.stringify({ days, duration_days, start_date, plan_id, name }) }),
