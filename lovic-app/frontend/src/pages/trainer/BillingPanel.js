...
export default function BillingPanel() {
  ...
  const [editing, setEditing] = useState(null); // clientId
  ...
  const upcoming = clients.filter(c => c.next_payment_date && String(c.next_payment_date).slice(0, 10) >= today)
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14, marginBottom: 24 }}>
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 28, fontWeight: 800, color: 'var(--coral)' }}>{totalMonthly.toFixed(0)} €</p>
        </div>
        <div className="card" style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 600, marginBottom: 6 }}>CLIENTAS ACTIVAS</p>
          <p style={{ fontSize: 28, fontWeight: 800 }}>{clients.length}</p>
        </div>
        <div className="card" style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 600, marginBottom: 6 }}>PRÓXIMOS PAGOS</p>
          <p style={{ fontSize: 28, fontWeight: 800, color: 'var(--gold)' }}>{upcoming.length}</p>
        </div>
      </div>
            ...
            return (
              <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  ...
                  {c.monthly_fee > 0 && <span style={{ fontWeight: 800, color: 'var(--coral)' }}>{Number(c.monthly_fee).toFixed(0)} €</span>}
                </div>
              </div>
            );
          })}
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          {clients.map(c => {
            ...
            return (
              <div key={c.id} style={{ borderBottom: '1px solid var(--border)', padding: '14px 0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontWeight: 700, marginBottom: 4 }}>{c.name}</p>
                    <p style={{ fontSize: 12, color: 'var(--muted)' }}>{c.email}</p>
                    {planEnd && (
                      <p style={{ fontSize: 12, marginTop: 4, color: planDaysLeft <= 0 ? '#dc2626' : planDaysLeft <= 7 ? '#ca8a04' : 'var(--muted)' }}>
                        {planDaysLeft <= 0 ? '⚠️ Plan vencido' : planDaysLeft <= 7 ? `⚠️ Plan vence en ${planDaysLeft} días` : `Plan hasta: ${planEnd.toLocaleDateString('es', { day: 'numeric', month: 'short', year: 'numeric' })}`}
                      </p>
                    )}
                    {c.notes && !isEditing && <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4, fontStyle: 'italic' }}>{c.notes}</p>}
                  </div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
                    {!isEditing && (
                      <>
                        {c.monthly_fee > 0 && <span style={{ fontWeight: 800, color: 'var(--coral)', fontSize: 15 }}>{Number(c.monthly_fee).toFixed(0)} €/mes</span>}
                        {c.next_payment_date && <span style={{ fontSize: 12, color: 'var(--muted)' }}>Pago: {new Date(String(c.next_payment_date).slice(0, 10) + 'T00:00:00').toLocaleDateString('es', { day: 'numeric', month: 'short' })}</span>}
                        <button className="btn-ghost" onClick={() => startEdit(c)} style={{ fontSize: 13, padding: '6px 12px' }}>✏️ Editar</button>
                      </>
                    )}
                  </div>
                </div>
